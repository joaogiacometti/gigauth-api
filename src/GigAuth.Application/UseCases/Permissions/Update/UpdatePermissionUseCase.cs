...
public class UpdatePermissionUseCase(
    IPermissionWriteOnlyRepository writeRepository,
    IPermissionReadOnlyRepository readRepository,
    IUnitOfWork unitOfWork) : IUpdatePermissionUseCase
{
    public async Task Execute(Guid id, RequestPermission request)
    {
        ...
        if (permissionWithPermissionName is not null && permissionToUpdate.Id != permissionWithPermissionName.Id)
            throw new AlreadyUsedException([ResourceErrorMessages.NAME_ALREADY_USED]);
